...
TebVisualizationPtr visual_obs;
...
int main( int argc, char** argv )
{
  ...
  ros::Timer cycle_timer = n.createTimer(ros::Duration(0.025), CB_mainCycle); //0.025
...
//  obst_vector.push_back( boost::make_shared<PointObstacle>(0,2.0) );
...
//  obst_vector.at(1)->setCentroidVelocity(vel);
  ...
  polyobst->pushBackVertex(0.5, 0.0);
  polyobst->pushBackVertex(6.0, 0.0);
  polyobst->pushBackVertex(6.0, -0.5);
  polyobst->pushBackVertex(0.5, -0.5);
  ...
  visual_obs = TebVisualizationPtr(new TebVisualization(n, config));
...
void CB_mainCycle(const ros::TimerEvent& e)
{
  planner->plan(PoseSE2(0,-4, 1.57), PoseSE2(4,1,0)); // hardcoded start and goal for testing purposes
}
...
{
  planner->visualize();
  visual_obs->publishObstacles(obst_vector);
  visual_obs->publishViaPoints(via_points);
}
...
void CreateInteractiveMarker(const double& init_x, const double& init_y, unsigned int id, std::string frame, interactive_markers::InteractiveMarkerServer* marker_server, interactive_markers::InteractiveMarkerServer::FeedbackCallback feedback_cb)
{
  ...
  i_marker.pose.orientation.x = 0.0;
  i_marker.pose.orientation.y = 0.0;
  i_marker.pose.orientation.z = 0.0;
