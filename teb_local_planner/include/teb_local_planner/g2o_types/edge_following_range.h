...
//    double dist = bandpt->pose().x()- (_measurement->position().x()+t_*follower_vel_->position().x());
    double dist = (bandpt->position() - _measurement->position()).norm();
...
//    _error[1] = penaltyBoundToInterval(bandpt->position().y() - _measurement->y(), -0.3, 0.3, cfg_->optim.penalty_epsilon);
    ...
    _error[0] = penaltyBoundFromBelow(dist, cfg_->follower.min_follower_dist, cfg_->optim.penalty_epsilon);
    _error[1] = penaltyBoundToInterval(dist, cfg_->follower.max_follower_dist, cfg_->optim.penalty_epsilon);
